...
@LYCIA.on_message(filters.command("OKU"))
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("OKU AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("OKU Is Best...")
    try:
        ...
        await m.edit(str(e))
        return
    await m.edit("Made By Blackest Lotus...")
